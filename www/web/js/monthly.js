...
(function($) {
	$.fn.extend({
		...
		if (options.weekStart == 'Сб') {
			$('#' + uniqueId).append('<div class="monthly-day-title-wrap"><div>Сб</div><div>Mon</div><div>Tue</div><div>Wed</div><div>Thu</div><div>Fri</div><div>Sat</div></div><div class="monthly-day-wrap"></div>');
		} else if (options.weekStart == 'Пн') {
			$('#' + uniqueId).append('<div class="monthly-day-title-wrap"><div>Пн</div><div>Вт</div><div>Ср</div><div>Чт</div><div>Пт</div><div>Сб</div><div>Вс</div></div><div class="monthly-day-wrap"></div>');
		} else {
		...
		$(document.body).on('click', '#'+uniqueId+' a.monthly-day', function (e) {
			...
			// If picker, pick date
				...
				if($(this).hasClass('monthly-past-day') && options.disablePast == true) {
					...
					$(''+options.target+'').val(whichDay+'/'+setMonth+'/'+setYear);
